...
def simulate_GRN_delay_frozen(
    ...
    # Get time parameters
    n_t_input = t_span.size
    n_t = t_span[::skip].size
    GRN_dt = t_span[skip] - t_span[0]
    
    # Get # cells
    n_c = x_frozen.shape[0]
    ...
    type_idx = assign_types(
        types, type_n_c, x0=x_frozen, type_method="center", center=center
    )
    
    # Define integration function using RHS
    def step_GRN_frozen(E, E_past, A, GRN_dt):
        E_bar = A @ E_past
    ...
    # Get cell-cell transition matrix
    A = x_to_transition_mtx(x_frozen, L)
    ...
    for step in iterator:
        ...
        E = step_GRN_frozen(E, E_past, A, GRN_dt)
        E_save[step] = E
...
# Set GRN time-span
GRN_tmax = 5
t0, tmax, n_t = df["t0"][0], df["tmax"][0], df["n_t"][0]
...
def simulate_frozen(args, skip=20, thresh=0.1, progress_bar=False, count_skip=5):
    """
    """
    x_idx, params = args
    
    # Extract data and metadata
    x_file, x_frozen = x_files[x_idx], x_saves_frozen[x_idx]
    x_file = os.path.split(x_file)[-1][:-4]
    metadata = metadf.loc[metadf["coords_fname"] == x_file,]
    metadata = metadata.iloc[0, :].to_dict()
    
    # Simulate signaling using DDE
    E_save = simulate_GRN_delay_frozen(
        GRN_t_span,
        rhs_delay=tc_rhs,
        dde_params=params,
        x_frozen=x_frozen,
    ...
    # Save GRN 
    prefix = (
        "alpha{0:.2f}_k{1:.2e}_p{2:.1f}_delta{3:.2f}_lambda{4:.1e}_tau{5:.2f}.npz"
    ).format(*params, delay)
    save_path = os.path.join(GRN_dir, x_file + "_Esave_" + prefix)
    np.savez_compressed(save_path, E_save)
...
# Get all combinations of lattices and GRN params
sim_space = [
    (idx, p) for idx, _ in enumerate(x_saves_frozen) for p in param_space
]

# Simulate
print(f"Simulating {len(sim_space)} runs on {cores} cores ({len(sim_space)/cores:.2f} per core)")
from multiprocessing import Pool
if __name__ == '__main__':
    with Pool(cores) as p:
        results = list(p.imap_unordered(simulate_frozen, sim_space))

# Convert metadata to DataFrame and save
resf = [os.path.split(t[2])[-1][:-4] for t in results]
res_coordsf = ["_".join(f.split("_")[:4]) for f in resf]

resdata = np.array([f.split("_") for f in resf]).T

p0_vals = [np.float32(d[2:]) for d in resdata[0]]
v0_vals = [np.float32(d[2:]) for d in resdata[1]]
dens_vals = [np.float32(d[4:]) for d in resdata[2]]
alpha_vals = [np.float32(d[5:]) for d in resdata[5]]
k_vals = [np.float32(d[1:]) for d in resdata[6]]
p_vals = [np.float32(d[1:]) for d in resdata[7]]
delta_vals = [np.float32(d[5:]) for d in resdata[8]]
lambda_vals = [np.float32(d[6:]) for d in resdata[9]]
tau_vals = [np.float32(d[3:]) for d in resdata[10]]

Esave_metadata = pd.DataFrame(dict(
    Esave_fname  = resf,
    coords_fname = res_coordsf,
    alpha        = alpha_vals, 
    k_s          = k_vals, 
    p_s          = p_vals, 
    delta        = delta_vals, 
...
Esave_metadata = Esave_metadata.merge(metadf)
Esave_metadata.to_csv("../data/2020-10-01_p0v0dens_phase_sims2/Esave/Esave_metadata.csv")
